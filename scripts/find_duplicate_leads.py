...
def writeCSV(type_name, items, ordered_keys):
	...
	f = open('%s %s Duplicates.csv' % (org_name, type_name), 'wt')
	try:
...
def getDuplicatesForLeadName(lead_name):
	...
	print "%s of %s: %s" % (keys_with_dupes_lead_name.index(lead_name) + 1, len(keys_with_dupes_lead_name), lead_name)

## Add to a list of duplicates for contact emails
def getDuplicatesForEmail(email):
	for dupe in emails[email]:
		email_duplicates.append({ 'Email Address': email, 'Lead Name': dupe['display_name'], 'Status Label': dupe['status_label'], 'Lead ID': dupe['id'], 'Lead Date Created': dupe['date_created'], 'Close URL': 'https://app.close.com/lead/%s/' % dupe['id'] })
	print "%s of %s: %s" % (keys_with_dupes_email.index(email) + 1, len(keys_with_dupes_email), email)
