...
def getTTR(user):
    if user != None:
        print(
            f"Getting all activities in the last {args.past_days} days for {user['user_full_name']}..."
        )
    else:
        print(
            f"Getting all activities in the last {args.past_days} days for {'All Users'}..."
        )
    ...
    while has_more:
        ...
        for activity in resp['data']:
            if (
                ...
                and activity['lead_id'] != None
            ):
                activity['date_created'] = (
                    activity['date_created'].split('+')[0].split('.')[0]
                )
